...
namespace {
    ...
    void compute_rest(const std::vector<float>& xyz, const std::vector<std::pair<u32, u32>>& edges, std::vector<float>& rest) {
        ...
        for (size_t k = 0; k < edges.size(); ++k) {
            const u32 i    = edges[k].first;
            const u32 j    = edges[k].second;
...
TEST_CASE("api_create_destroy_step_map") {
    ...
    ExecPolicy ex{};
    SolvePolicy sv{};
    Handle h = create(InitDesc{std::span<const float>(xyz.data(), xyz.size()), std::span<const u32>(tris.data(), tris.size()), std::span<const u32>(fixed.data(), fixed.size()), ex, sv});
    REQUIRE(h != nullptr);
...
TEST_CASE("fixed_vertices_remain_pinned") {
    ...
    SolvePolicy sv{};
    sv.iterations = 10;
    auto h        = create(InitDesc{std::span<const float>(xyz.data(), xyz.size()), std::span<const u32>(tris.data(), tris.size()), std::span<const u32>(fixed.data(), fixed.size()), ex, sv});
...
TEST_CASE("constraints_converge_from_perturbation") {
    ...
    compute_rest(xyz, edges, rest);
    ExecPolicy ex{};
    SolvePolicy sv{};
    sv.iterations         = 20;
    sv.substeps           = 1;
    sv.compliance_stretch = 0.0f;
    auto h                = create(InitDesc{std::span<const float>(xyz.data(), xyz.size()), std::span<const u32>(tris.data(), tris.size()), std::span<const u32>(fixed.data(), fixed.size()), ex, sv});
    auto v                = map_dynamic(h);
    for (size_t i = 0; i < v.count; ++i)
        if (std::find(fixed.begin(), fixed.end(), static_cast<u32>(i)) == fixed.end()) v.pos_y[i] += 0.05f;
    StepParams sp{};
    sp.gravity_x = sp.gravity_y = sp.gravity_z = 0.0f;
...
TEST_CASE("compliance_softens_solution") {
    ...
    auto va                  = map_dynamic(ha);
    auto vb                  = map_dynamic(hb);
    for (size_t i = 0; i < va.count; ++i)
        if (std::find(fixed.begin(), fixed.end(), static_cast<u32>(i)) == fixed.end()) {
            va.pos_y[i] += 0.03f;
            vb.pos_y[i] += 0.03f;
        }
    StepParams sp{};
    ...
    const double ea = rms_edge_error(va, edges, rest);
    const double eb = rms_edge_error(vb, edges, rest);
    CHECK(ea < eb);
    destroy(ha);
    destroy(hb);
}
...
TEST_CASE("determinism_same_setup") {
    ...
    std::vector<u32> tris;
    std::vector<u32> fixed;
    make_grid(12, 12, 0.07f, xyz, tris, fixed);
    SolvePolicy solve{};
    solve.iterations = 12;
    solve.substeps   = 2;
...
TEST_CASE("damping_reduces_kinetic_energy") {
    ...
    std::vector<u32> tris;
    std::vector<u32> fixed;
    make_grid(10, 10, 0.06f, xyz, tris, fixed);
    SolvePolicy solve{};
    solve.iterations = 1;
    solve.damping    = 0.2f;
    auto h           = create(InitDesc{xyz, tris, fixed, ExecPolicy{}, solve});
    auto v           = map_dynamic(h);
    ...
    auto kinetic                               = [&](const DynamicView& dv) {
        ...
        for (size_t i = 0; i < dv.count; ++i) {
            const double vx = dv.vel_x[i], vy = dv.vel_y[i], vz = dv.vel_z[i];
            k += 0.5 * (vx * vx + vy * vy + vz * vz);
        }
        return k;
    };
    const double ke0 = kinetic(v);
    step(h, sp);
    const double ke1 = kinetic(v);
    CHECK(ke1 < ke0);
    destroy(h);
}
...
TEST_CASE("backend_request_simd_matches_native") {
    ...
    std::vector<u32> tris;
    std::vector<u32> fixed;
    make_grid(10, 10, 0.05f, xyz, tris, fixed);
    SolvePolicy solve{};
    solve.iterations = 6;
    solve.substeps   = 2;
    ExecPolicy exN{};
    exN.backend = ExecPolicy::Backend::Native;
    ExecPolicy exA{};
    exA.backend = ExecPolicy::Backend::Simd;
    ...
    auto hA     = create(InitDesc{std::span<const float>(xyz.data(), xyz.size()), std::span<const u32>(tris.data(), tris.size()), std::span<const u32>(fixed.data(), fixed.size()), exA, solve});
    REQUIRE(hN != nullptr);
    REQUIRE(hA != nullptr);
    auto vN = map_dynamic(hN);
    auto vA = map_dynamic(hA);
    for (int i = 0; i < 60; ++i) {
        step(hN, StepParams{});
        step(hA, StepParams{});
...
TEST_CASE("backend_request_tbb_matches_native") {
    ...
    exN.backend = ExecPolicy::Backend::Native;
    ExecPolicy exT{};
    exT.backend = ExecPolicy::Backend::Tbb;
    auto hN     = create(InitDesc{std::span<const float>(xyz.data(), xyz.size()), std::span<const u32>(tris.data(), tris.size()), std::span<const u32>(fixed.data(), fixed.size()), exN, solve});
...
TEST_CASE("tbb_threads_variants_match") {
    ...
    solve.substeps   = 2;
    ExecPolicy exA{};
    exA.backend = ExecPolicy::Backend::Tbb;
    exA.threads = 0;
    ExecPolicy exB{};
    exB.backend = ExecPolicy::Backend::Tbb;
    exB.threads = 2;
