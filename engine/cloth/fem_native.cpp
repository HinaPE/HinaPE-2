...
namespace HinaPE::detail {
    ...
    struct FEMStaticState {
        usize n_verts{0};
        std::pmr::vector<u32> e_i, e_j;
        std::pmr::vector<float> w_e;
        std::pmr::vector<float> deg_w;
        SolvePolicy solve{};
        ExecPolicy exec{};
        explicit FEMStaticState(std::pmr::memory_resource* mr) : e_i(mr), e_j(mr), w_e(mr), deg_w(mr) {}
    };
    ...
    struct FEMDynamicState {
        ...
        explicit FEMDynamicState(std::pmr::memory_resource* mr) : data(mr), prev_x(mr), prev_y(mr), prev_z(mr), x(mr), r(mr), z(mr), p(mr), Ap(mr), rhs(mr), diagA(mr), mass(mr) {}
    };

    static inline float safe_cot(const float ax, const float ay, const float az, const float bx, const float by, const float bz) noexcept {
    ...
    private:
        ...
        void build_cotangent_edges_(std::span<const u32> tris, std::span<const float> xyz) {
            ...
            for (usize t = 0; t < m; ++t) {
                ...
                float v0x = jx - ix, v0y = jy - iy, v0z = jz - iz;
                float w0x = kx - ix, w0y = ky - iy, w0z = kz - iz;
                float v1x = kx - jx, v1y = ky - jy, v1z = kz - jz;
                float w1x = ix - jx, w1y = iy - jy, w1z = iz - jz;
                float v2x = ix - kx, v2y = iy - ky, v2z = iz - kz;
                float w2x = jx - kx, w2y = jy - ky, w2z = jz - kz;
                ...
                add_weight(j, k, 0.5f * cot_i);
                add_weight(k, i, 0.5f * cot_j);
                add_weight(i, j, 0.5f * cot_k);
            }
            std::sort(accum.begin(), accum.end(), [](const auto& a, const auto& b) { return a.first < b.first; });
            st.e_i.clear();
            st.e_j.clear();
            st.w_e.clear();
            st.deg_w.assign(st.n_verts, 0.0f);
            for (usize idx = 0; idx < accum.size();) {
                ...
                do {
                    w += accum[idx].second;
                    ++idx;
                } while (idx < accum.size() && accum[idx].first == key);
                if (w <= 0.0f) continue;
                st.e_i.push_back(key.first);
                st.e_j.push_back(key.second);
                st.w_e.push_back(w);
        ...
        void apply_A_(const float* in, float* out) const noexcept {
            const usize n = st.n_verts;
            const usize m = st.e_i.size();

            for (usize i = 0; i < n; ++i) out[i] = dy.mass[i] * in[i];

            const float s = dt2k_;
            for (usize e = 0; e < m; ++e) {
                const u32 a = st.e_i[e], b = st.e_j[e];
                const float w = st.w_e[e];
        ...
        void pcg_solve_axis_(std::pmr::vector<float>& axis) noexcept {
            const usize n = st.n_verts;
            auto& x       = dy.x;
            x.assign(axis.begin(), axis.begin() + n);
            auto& r  = dy.r;
            auto& z  = dy.z;
            auto& p  = dy.p;
            auto& Ap = dy.Ap;
