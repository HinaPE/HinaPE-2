...
namespace HinaPE::detail {
    ...
    struct TbbDynamicState {
        ...
        explicit TbbDynamicState(std::pmr::memory_resource* mr) : data(mr), prev_x(mr), prev_y(mr), prev_z(mr), lambda(mr) {}
    };
    ...
    public:
        ...
        void init(const InitDesc& desc) {
            ...
            if (st.exec.threads > 0) {
                control = std::make_unique<tbb::global_control>(tbb::global_control::max_allowed_parallelism, static_cast<size_t>(st.exec.threads));
            } else {
                control.reset();
            }
            ...
            const usize n_pad = dy.data.px.size();
            {
                ...
                tbb::parallel_for(
                    tbb::blocked_range<usize>(0, n_pad),
                    [&](const tbb::blocked_range<usize>& r) {
