...
namespace hwy {
    namespace HWY_NAMESPACE {
        ...
        static void HwyPredictPositions(const float* HWY_RESTRICT gravity, float* HWY_RESTRICT px, float* HWY_RESTRICT py, float* HWY_RESTRICT pz, float* HWY_RESTRICT vx, float* HWY_RESTRICT vy, float* HWY_RESTRICT vz, const float* HWY_RESTRICT inv_mass, float* HWY_RESTRICT prev_x, float* HWY_RESTRICT prev_y, float* HWY_RESTRICT prev_z, float dt, size_t n) {
            const Df d;
        ...
        static void HwyIntegrate(float* HWY_RESTRICT px, float* HWY_RESTRICT py, float* HWY_RESTRICT pz, float* HWY_RESTRICT vx, float* HWY_RESTRICT vy, float* HWY_RESTRICT vz, const float* HWY_RESTRICT inv_mass, const float* HWY_RESTRICT prev_x, const float* HWY_RESTRICT prev_y, const float* HWY_RESTRICT prev_z, float dt, float damping, size_t n) {
            const Df d;
            const auto zero   = Zero(d);
        ...
        static void HwySolveStretchNC(const uint32_t* HWY_RESTRICT batch, size_t batch_size, const uint32_t* HWY_RESTRICT e_i, const uint32_t* HWY_RESTRICT e_j, const float* HWY_RESTRICT rest, float* HWY_RESTRICT px, float* HWY_RESTRICT py, float* HWY_RESTRICT pz, const float* HWY_RESTRICT inv_mass) {
            const Df d;
            const Du32 du;
            const Dsi dsi;
            const auto zero = Zero(d);
        ...
        static void HwySolveStretchXPBD(const uint32_t* HWY_RESTRICT batch, size_t batch_size, const uint32_t* HWY_RESTRICT e_i, const uint32_t* HWY_RESTRICT e_j, const float* HWY_RESTRICT rest, float* HWY_RESTRICT px, float* HWY_RESTRICT py, float* HWY_RESTRICT pz, const float* HWY_RESTRICT inv_mass, float* HWY_RESTRICT lambda, float alpha) {
            const Df d;
            const Du32 du;
            const Dsi dsi;
            const auto zero   = Zero(d);
...
namespace HinaPE::detail {
    ...
    struct SimdDynamic {
        ...
        explicit SimdDynamic(std::pmr::memory_resource* mr) : data(mr), prev_x(mr), prev_y(mr), prev_z(mr), lambda(mr) {}
    };
    ...
    private:
        ...
        void predict_positions(const StepParams& p, f32 dt) noexcept {
            ...
            hwy::HWY_NAMESPACE::HwyPredictPositions(g, dy.data.px.data(), dy.data.py.data(), dy.data.pz.data(), dy.data.vx.data(), dy.data.vy.data(), dy.data.vz.data(), dy.data.inv_mass.data(), dy.prev_x.data(), dy.prev_y.data(), dy.prev_z.data(), dt, n);
#else
            (void) p;
            (void) dt;
            (void) n;
        ...
        void solve_stretch_batch(const std::pmr::vector<u32>& batch, f32 compliance, f32 dt) noexcept {
            ...
            hwy::HWY_NAMESPACE::HwySolveStretchXPBD(batch.data(), batch.size(), dy.data.e_i.data(), dy.data.e_j.data(), dy.data.rest_len.data(), dy.data.px.data(), dy.data.py.data(), dy.data.pz.data(), dy.data.inv_mass.data(), dy.lambda.data(), alpha);
#else
            (void) batch;
            (void) compliance;
            (void) dt;
...
#if defined(HINAPE_HAVE_SIMD)
            if (batch.empty()) return;
            hwy::HWY_NAMESPACE::HwySolveStretchNC(batch.data(), batch.size(), dy.data.e_i.data(), dy.data.e_j.data(), dy.data.rest_len.data(), dy.data.px.data(), dy.data.py.data(), dy.data.pz.data(), dy.data.inv_mass.data());
#else
            (void) batch;
#endif
        }
        ...
        void integrate(f32 dt, f32 damping) noexcept {
            const usize n = dy.data.inv_mass.size();
#if defined(HINAPE_HAVE_SIMD)
            hwy::HWY_NAMESPACE::HwyIntegrate(dy.data.px.data(), dy.data.py.data(), dy.data.pz.data(), dy.data.vx.data(), dy.data.vy.data(), dy.data.vz.data(), dy.data.inv_mass.data(), dy.prev_x.data(), dy.prev_y.data(), dy.prev_z.data(), dt, damping, n);
#else
            (void) dt;
            (void) damping;
            (void) n;
