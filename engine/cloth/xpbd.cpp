...
constexpr bool operator==(const aligned_allocator<T,A>&, const aligned_allocator<U,B>&) noexcept { return A==B; }
...
inline void greedy_edge_coloring(usize n, const avec<std::pair<u32,u32>>& edges, const std::vector<avec<u32>>& adj, avec<int>& color, std::vector<avec<u32>>& batches) {
    ...
    std::sort(order.begin(), order.end(), [&](u32 a, u32 b){ auto [ai,aj]=edges[a]; auto [bi,bj]=edges[b]; usize da = std::max(deg[ai],deg[aj]); usize db = std::max(deg[bi],deg[bj]); if (da!=db) return da>db; return a<b; });
...
inline void build_states(const InitDesc& desc, std::unique_ptr<Sim>& sim) {
    ...
    s->dy.pos_y.resize(n);
    s->dy.pos_z.resize(n);
    s->dy.prev_x.resize(n);
    s->dy.prev_y.resize(n);
    s->dy.prev_z.resize(n);
    s->dy.vel_x.resize(n);
    s->dy.vel_y.resize(n);
    s->dy.vel_z.resize(n);
    ...
    for (usize i = 0; i < n; ++i) {
        ...
        s->dy.prev_x[i] = s->dy.pos_x[i];
        s->dy.prev_y[i] = s->dy.pos_y[i];
        s->dy.prev_z[i] = s->dy.pos_z[i];
        s->dy.vel_x[i] = 0.0f;
        s->dy.vel_y[i] = 0.0f;
        s->dy.vel_z[i] = 0.0f;
...
inline void step_native(struct Sim& sim, const StepParams& params) {
    ...
    f32 dt = params.dt > 0.0f ? params.dt / static_cast<f32>(sub) : f32(1.0f/60.0f) / static_cast<f32>(sub);
